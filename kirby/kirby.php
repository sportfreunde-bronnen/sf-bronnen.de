...
class Kirby
{
    ...
    public function configure()
    {
        ...
        $allowed = array_filter(dir::read($root), function ($file) {
            return substr($file, 0, 7) === 'config.' and substr($file, -4)
                === '.php';
        });
    ...
    public function routes($routes = array())
    {
        ...
        if (!empty($routes) and is_array($routes)) {
            return $this->options['routes']
                = array_merge($this->options['routes'], $routes);
        }
        ...
        $otherRoute = function ($path = null) use ($site, $kirby) {
            ...
            if ($kirby->option('language.detect') && $kirby->route->lang
                && (!$path || $path === '/')) {
                ...
                if (get('language') === 'switch') {
                    ...
                    if (s::get('kirby_language') and
                        $language = $site->sessionLanguage()) {
                        // $language is already set but the user wants to
                        // select another language
                        $referer = r::referer();
                        if (!empty($referer)
                            && str::startsWith($referer,
                                $this->urls()->index())) {
                            $language = $kirby->route->lang;
                        }
                    } else {
            ...
            if ($site->representation !== null) {
                ...
                if ($site->representation === ''
                    || $site->representation != $page->representation()) {
        ...
        $routes['pluginAssets'] = array(
            ...
            'action'  => function ($plugin, $path) use ($kirby) {
                $errorResponse = new Response('The file could not be found',
                    'txt', 404);

                // filter out plugin names that contain directory traversal attacks
                if (preg_match('{[\\\\/]}', urldecode($plugin))) {
                ...
                $pluginRoot = $kirby->roots()->plugins() . DS . $plugin . DS
                    . 'assets';
                ...
                if (!str::startsWith(realpath($fileRoot),
                    realpath($pluginRoot))) {
                    return $errorResponse;
                }

                // success, serve the file
                return new Response(f::read($fileRoot),
        ...
        if ($site->multilang()) {
            ...
            foreach ($site->languages()->sortBy('isRoot', 'asc') as $lang) {
                $pattern = ($lang->path()) ? $lang->path() . '/(:all?)'
                    : '(:all)';
                $routes[] = array(
                    'pattern' => $pattern,
                    'host'    => $lang->host(),
            ...
            $routes['others'] = array(
                'pattern' => '(.*)',
                // this can't be (:all) to avoid overriding the actual language route
                'method'  => 'ALL',
    ...
    public function plugin($name, $mode = 'dir')
    {
        ...
        if (file_exists($file)) {
            return $this->plugins[$name] = include_once($file);
        }
    ...
    public function localize()
    {
        ...
        if ($site->multilang()) {
            ...
            $path = $this->roots()->languages() . DS . $site->language()
                    ->code();

            // load .php file if it exists
            if (f::exists($path . '.php')) {
                include_once($path . '.php');
            }

            // load .yml file and set as language variables if it exists
            if (f::exists($path . '.yml')) {
                l::set(data::read($path . '.yml', 'yaml'));
            }
        }
    ...
    public function branch()
    {
        ...
        // which branch?
        $branch = count($this->options['languages']) > 0 ? 'multilang'
            : 'default';
    ...
    public function cache()
    {
        ...
        if ($this->options['cache']) {
            if ($this->options['cache.driver'] == 'file'
                and empty($this->options['cache.options'])) {
                $this->options['cache.options'] = array(
                    'root' => $this->roots()->cache()
                );
            }
            return $this->cache = cache::setup($this->options['cache.driver'],
    ...
    public function render(Page $page, $data = array(), $headers = true)
    {
        ...
        if ($this->options['cache'] && $page->isCachable()
            && in_array(r::method(), ['GET', 'HEAD'])) {
            ...
            if ($this->options['cache.autoupdate'] and $this->cache()
                    ->exists($cacheId)) {
    ...
    public function launch()
    {
        ...
        if (
            $this->option('language.detect') && $this->site()->multilang()
            && $this->site()->language()
        ) {
    ...
    public function trigger($event, $args = null)
    {
        ...
        foreach (static::$hooks as $pattern => $hooks) {
            ...
            foreach ($hooks as $key => $callback) {
                if (!array_key_exists($pattern, static::$triggered)) {
                    static::$triggered[$pattern] = array();
                }
                if (in_array($key, static::$triggered[$pattern])) {
                    continue;
                }
                ...
                if (is_string($callback)) {
                    $callback
                        = (new ReflectionFunction($callback))->getClosure();
                }
    ...
    public function component($name, $component = null)
    {
        if (is_null($component)) {
            if (!isset($this->components[$name])) {
                ...
                if (file_exists(__DIR__ . DS . 'kirby' . DS . 'component' . DS
                    . strtolower($name) . '.php')) {
                    $this->component($name, 'Kirby\\Component\\' . $name);
                } else {
                    throw new Exception('The component "' . $name
                        . '" does not exist');
                }
            }
            ...
            if (!is_a($object, 'Kirby\\Component')) {
                throw new Exception('The component "' . $name
                    . '" must be an instance of the Kirby\\Component class');
            }

            if (!is_a($object, 'Kirby\\Component\\' . $name)) {
                throw new Exception('The component "' . $name
                    . '" must be an instance of the Kirby\\Component\\'
                    . ucfirst($name) . ' class');
            }
